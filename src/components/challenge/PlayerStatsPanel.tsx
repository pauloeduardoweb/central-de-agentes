...
import { Trophy, Award, TrendingUp, Users, Flame, RefreshCw, Edit3, ShieldCheck, Crown } from 'lucide-react';
...
export const PlayerStatsPanel: React.FC<PlayerStatsPanelProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      {isMaster ? (
        <div className="p-6 rounded-3xl bg-gradient-to-br from-[#0c1e38] via-[#091528] to-[#040d1a] border border-amber-500/50 shadow-2xl text-white relative overflow-hidden">
          <div className="flex items-center space-x-3 border-b border-slate-800 pb-4 mb-4">
            <div className="w-12 h-12 rounded-2xl bg-amber-500/10 border border-amber-500/30 flex items-center justify-center text-amber-400">
              <Crown className="w-6 h-6" />
            </div>
            <div>
              <div className="inline-flex items-center space-x-1.5 px-2.5 py-0.5 rounded-full bg-amber-950 text-amber-300 border border-amber-500/30 text-[10px] font-black uppercase tracking-wider">
          ...
          <div className="p-3.5 rounded-2xl bg-amber-950/20 border border-amber-500/30 text-xs text-amber-200/90 leading-relaxed space-y-1">
            <p className="font-bold">Conta Administradora Ativa (MASTER)</p>
            <p className="text-[11px] text-slate-300">
              Contas Master possuem acesso administrativo completo e não acumulam XP nem participam do ranking de alunos.
            </p>
          </div>
        </div>
      ) : (
        <div className="p-6 rounded-3xl bg-gradient-to-br from-[#0a1b2d] via-[#081524] to-[#030e1a] border border-cyan-500/50 shadow-2xl shadow-cyan-950/60 text-white relative overflow-hidden">
          
          <div className="flex items-center justify-between border-b border-slate-800 pb-4 mb-4">
            <div className="flex items-center space-x-3">
              <UserAvatar username={username || 'Aluno'} avatarUrl={avatar} size="lg" />
              <div>
                <span className="text-[10px] font-black uppercase tracking-widest text-cyan-400 bg-cyan-950 px-2 py-0.5 rounded-full border border-cyan-500/30">
                  PERFIL DO ALUNO
                </span>
            ...
            {!isMaster && (
              <button
                ...
                className="p-2 rounded-xl bg-slate-900 hover:bg-slate-800 border border-cyan-500/30 text-cyan-300 hover:text-white text-xs font-bold flex items-center space-x-1.5 transition-all cursor-pointer"
      ...
      {/* RANKING GERAL */}
      <div className="p-5 rounded-3xl bg-gradient-to-br from-[#0a192f] via-[#081220] to-[#040a14] border border-cyan-500/40 shadow-xl text-white space-y-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              className="p-1 rounded-lg bg-slate-900 hover:bg-slate-800 text-cyan-400 border border-cyan-500/30 transition-all cursor-pointer"
            >
              <RefreshCw className={`w-3.5 h-3.5 ${isLoading ? 'animate-spin' : ''}`} />
            </button>
            <button 
              onClick={onOpenHowItWorks}
              className="text-[10px] text-cyan-300/80 hover:text-cyan-200 underline font-bold cursor-pointer"
            >
              Ver regras
            </button>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="p-6 text-center text-xs text-slate-300 bg-slate-900/60 rounded-2xl border border-slate-800 space-y-2">
