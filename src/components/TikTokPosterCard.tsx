...
export const TikTokPosterCard: React.FC<TikTokPosterCardProps> = ({
  ...
  const renderPosterGraphic = () => {
    ...
    switch (agent.posterSlug) {
      ...
      default: {
        const bgImg = agent.coverImage || 'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?auto=format&fit=crop&w=1000&q=80';
            ...
            <button
              ...
              className={`w-full py-2.5 px-4 rounded-xl bg-gradient-to-r ${
                agent.chatGptUrl.includes('wa.me')
                  ...
                  : theme.btnGradient
              } text-white font-black text-xs flex items-center justify-center space-x-2 shadow-lg transform group-hover:scale-105 transition-all`}
            >
              {agent.chatGptUrl.includes('wa.me') ? (
                <>
