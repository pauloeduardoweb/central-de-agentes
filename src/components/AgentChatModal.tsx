...
import { X, Send, Trash2, Copy, Check, Bot, User, Sparkles, RefreshCw, ChevronDown, ChevronUp, Code2, Globe, FileJson, AlertCircle, ExternalLink, Camera, Image as ImageIcon } from 'lucide-react';
        ...
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-slate-200 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-900/80">
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              className="px-2.5 py-1.5 rounded-lg text-xs font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-200/60 dark:hover:bg-slate-800 flex items-center space-x-1 transition-colors"
            >
        ...
        {showInstructions && (
          <div className="p-4 bg-slate-100 dark:bg-slate-800/90 border-b border-slate-200 dark:border-slate-700 text-xs text-slate-700 dark:text-slate-300 space-y-2 animate-in slide-in-from-top duration-150">
            <div className="flex items-center justify-between font-semibold text-slate-900 dark:text-white">
              <span>Instruções do Sistema (System Instructions):</span>
              <span className="text-[10px] text-slate-500 font-normal">Temperatura: {agent.temperature}</span>
            </div>
            <pre className="whitespace-pre-wrap font-mono text-[11px] bg-white dark:bg-slate-900 p-3 rounded-lg border border-slate-200 dark:border-slate-700/80 max-h-36 overflow-y-auto leading-relaxed">
