...
export const AgentCard: React.FC<AgentCardProps> = ({
  ...
  return (
    ...
    >
            ...
            <div className="relative">
              ...
              {showMenu && (
                <>
                  ...
                  <div className="absolute right-0 top-8 z-20 w-44 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-xl py-1 text-xs">
                    <button
                      onClick={() => {
                        setShowMenu(false);
                        onEdit(agent);
                      }}
                      className="w-full px-3 py-2 text-left text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 flex items-center space-x-2"
                    >
                    ...
                    {agent.isCustom && (
                      <button
                        ...
                        className="w-full px-3 py-2 text-left text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/50 flex items-center space-x-2 border-t border-slate-100 dark:border-slate-700/60 mt-1 pt-1.5"
                      >
      ...
      <div>
        <div className="flex items-center justify-between pt-3 border-t border-slate-100 dark:border-slate-800/80 mb-3">
          <div className="flex items-center space-x-1.5 text-slate-400">
            {agent.capabilities?.codeInterpreter && (
              <span title="Code Interpreter" className="p-1 rounded bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300">
                <Code2 className="w-3.5 h-3.5" />
              </span>
            )}
            {agent.capabilities?.webSearch && (
              <span title="Web Search" className="p-1 rounded bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300">
                <Globe className="w-3.5 h-3.5" />
              </span>
            )}
            {agent.capabilities?.imageGeneration && (
              <span title="Geração de Imagens" className="p-1 rounded bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300">
                <Image className="w-3.5 h-3.5" />
              </span>
            )}
            {agent.capabilities?.jsonOutput && (
              <span title="JSON Estruturado" className="p-1 rounded bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300">
