...
export const AgentPrimaryAction: React.FC<AgentPrimaryActionProps> = ({ agent, onOpenLocal }) => {
  return (
    <button
      ...
      className="group relative w-full text-left p-3 sm:p-3.5 rounded-2xl bg-gradient-to-r from-cyan-600 via-blue-600 to-teal-600 border-2 border-cyan-300 hover:border-white shadow-[0_8px_25px_rgba(6,182,212,0.3)] hover:shadow-[0_10px_32px_rgba(6,182,212,0.45)] hover:scale-[1.01] transition-all duration-200 cursor-pointer overflow-hidden z-10 active:scale-[0.99]"
    >
      ...
      <div className="absolute inset-0 opacity-30 pointer-events-none overflow-hidden rounded-2xl">
        <div className="w-1/2 h-full bg-gradient-to-r from-transparent via-white/40 to-transparent transform -skew-x-12 -translate-x-full animate-[shimmer_1.4s_ease-out_1_forwards]" />
      </div>
      ...
      <div className="relative z-10 flex items-center justify-between gap-3">
        <div className="space-y-1 min-w-0">
          ...
          <div className="flex items-center flex-wrap gap-1">
            <span className="px-2 py-0.5 rounded-full bg-white text-cyan-950 text-[8.5px] font-black uppercase tracking-wider shadow-xs">
              ★ RECOMENDADO
            </span>
            <span className="px-2 py-0.5 rounded-full bg-cyan-950/40 text-cyan-100 border border-cyan-200/40 text-[8.5px] font-extrabold uppercase">
              CHAT LOCAL
            </span>
          </div>
          ...
          <div>
            ...
            <p className="text-[11px] text-cyan-50/95 font-medium leading-tight truncate">
