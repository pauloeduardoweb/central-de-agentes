...
export const ProductMinerPage: React.FC<ProductMinerPageProps> = ({
  ...
  const [totalStoredBaseProducts, setTotalStoredBaseProducts] = useState<number>(0);
  ...
  const loadCategories = async () => {
    ...
    try {
      const data = await fetchCollectorCategories(studentCode);
      setCollectorCategories(data.categories || []);
      if (typeof data.totalStoredProducts === 'number') {
        setTotalStoredBaseProducts(data.totalStoredProducts);
      }
    } catch (err: any) {
              ...
              <div className="rounded-2xl border border-amber-300 bg-gradient-to-r from-amber-50 via-orange-50/50 to-amber-50 p-4 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <div className="flex items-center gap-2 text-amber-900 font-extrabold text-sm">
                    ...
                    <span>Organização Automática da Base Existente ({totalStoredBaseProducts > 0 ? `~${totalStoredBaseProducts.toLocaleString('pt-BR')} produtos` : 'Base MySQL'})</span>
                  </div>
