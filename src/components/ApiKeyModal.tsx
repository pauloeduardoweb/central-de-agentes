...
export const ApiKeyModal: React.FC<ApiKeyModalProps> = ({ onClose, onSave, isMandatoryOnboarding = false }) => {
  ...
  return (
    ...
    >
      <style>{`
        .login-bg-responsive {
          background-image: url('/login-bg-hd.png'), url('https://i.postimg.cc/300YdHdZ/CAPA.png'), url('https://i.postimg.cc/DnDYSZ00/CAPA.png');
      ...
      <div className="bg-[#030b18]/95 border-2 border-blue-500/50 rounded-2xl w-[76%] max-w-[250px] sm:max-w-md overflow-hidden shadow-[0_0_50px_rgba(37,99,235,0.4)] relative z-10 text-slate-100 transform translate-y-12 sm:translate-y-8 transition-all duration-300 my-auto sm:my-0">
        
        {/* High Tech Cover Banner */}
        <div className="relative w-full aspect-[16/5.5] sm:aspect-[16/5] bg-[#020713] overflow-hidden border-b border-blue-500/40">
          <svg className="w-full h-full object-cover" viewBox="0 0 1200 480" fill="none">
            ...
            <g filter="url(#modalGlow)">
              <text x="600" y="250" textAnchor="middle" fill="url(#modalTextMetallic)" stroke="#60a5fa" strokeWidth="2.6" fontSize="88" fontWeight="900" fontFamily="sans-serif" letterSpacing="4">
                GERAÇÃO Z PRO
              </text>
            </g>
        ...
        <div className="p-2.5 sm:p-6 pt-1 space-y-2 sm:space-y-4">
          {!isMandatoryOnboarding && (
            <button
              onClick={onClose}
              className="absolute top-2 right-2 text-slate-400 hover:text-white p-1 rounded-lg hover:bg-slate-800 transition-colors z-20"
            >
          ...
          <div className="flex items-center space-x-2 sm:space-x-3">
            <div className="w-7 h-7 sm:w-11 sm:h-11 rounded-xl bg-gradient-to-tr from-cyan-500 to-blue-600 flex items-center justify-center text-slate-950 font-bold shadow-lg shadow-cyan-500/20 shrink-0">
              <Lock className="w-3 h-3 sm:w-5 sm:h-5 text-white" />
            </div>
            <div>
              <h2 className="text-[11px] sm:text-lg font-bold text-white flex items-center space-x-2">
        ...
        {/* Notice Banner */}
        <div className="bg-cyan-950/40 border border-cyan-500/30 rounded-xl p-2 sm:p-3.5 text-[9px] sm:text-xs text-cyan-200/90 leading-relaxed space-y-1">
          <div className="flex items-center space-x-1 font-bold text-cyan-400">
            <ShieldCheck className="w-3 h-3 sm:w-4 sm:h-4 shrink-0" />
            <span>Área Restrita aos Alunos</span>
          </div>
          <p className="line-clamp-2 sm:line-clamp-none">
            Insira o seu <b>Código de Acesso Exclusivo</b> para liberar toda a plataforma.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-2 sm:space-y-4">
          <div>
            <label className="block text-[9px] sm:text-xs font-semibold text-slate-300 mb-1 flex items-center justify-between">
              <span className="flex items-center space-x-1">
                <Key className="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5 text-cyan-400" />
                <span>Código de Acesso:</span>
              </span>
              <span className="text-[8px] sm:text-[10px] text-cyan-400 font-normal">Na mentoria</span>
            </label>
            <input
              type="text"
              placeholder="ex: GZ-XXXX-XXXX"
              value={accessCode}
              onChange={(e) => {
              ...
              autoFocus
              className="w-full px-2.5 py-1.5 sm:px-3.5 sm:py-2.5 rounded-lg sm:rounded-xl bg-[#020d14] border border-amber-500/40 text-[10px] sm:text-xs font-mono text-white focus:outline-none focus:ring-2 focus:ring-amber-500"
            />
          </div>

          {error && (
            <p className="text-[10px] sm:text-xs text-rose-400 font-medium mt-1 flex items-center space-x-1">
              <AlertCircle className="w-3 h-3 sm:w-3.5 sm:h-3.5 shrink-0" />
              <span>{error}</span>
            </p>
          )}

          <div className="flex items-center justify-end space-x-2 pt-0.5 sm:pt-2">
            {!isMandatoryOnboarding && (
              <button
                type="button"
                onClick={onClose}
                className="px-2.5 py-1.5 sm:px-4 sm:py-2.5 rounded-lg sm:rounded-xl text-[10px] sm:text-xs font-semibold text-slate-400 hover:text-white hover:bg-slate-800 transition-colors"
              >
                Cancelar
              </button>
            )}
            <button
              type="submit"
              disabled={loading}
              className="w-full sm:w-auto px-4 py-1.5 sm:px-6 sm:py-2.5 rounded-lg sm:rounded-xl text-[10px] sm:text-xs font-bold bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-slate-950 shadow-lg shadow-emerald-500/25 transition-all flex items-center justify-center space-x-1.5 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? (
                <>
