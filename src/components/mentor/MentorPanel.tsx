...
export const MentorPanel: React.FC<MentorPanelProps> = ({ studentCode, onBackToHub, onTabChange }) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      {/* Mentor Hero Banner */}
      <div className="relative rounded-2xl bg-gradient-to-r from-[#031d2e] via-[#02131c] to-[#042133] border border-cyan-500/30 p-6 md:p-8 shadow-2xl shadow-cyan-500/10 overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="space-y-2">
            <div className="flex items-center space-x-2">
              <span className="px-3 py-1 rounded-full text-xs font-bold bg-cyan-950/90 text-cyan-300 border border-cyan-500/40 flex items-center space-x-1.5 shadow-md">
                <Crown className="w-3.5 h-3.5 text-amber-400" />
                <span>Painel do Mentor</span>
              </span>
              <span className="px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-emerald-950/80 text-emerald-300 border border-emerald-800">
          ...
          {onBackToHub && (
            <button
              onClick={onBackToHub}
              className="px-4 py-2.5 rounded-xl bg-slate-900/80 hover:bg-slate-800 text-cyan-300 hover:text-white border border-cyan-500/30 text-xs font-bold flex items-center space-x-2 transition-all shrink-0"
            >
          ...
          return (
            ...
            >
              <div>
                ...
                <h3 className="font-bold text-base text-white group-hover:text-cyan-200 transition-colors">
                  {card.title}
                </h3>
